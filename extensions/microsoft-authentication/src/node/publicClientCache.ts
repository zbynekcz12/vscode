...
export class CachedPublicClientApplicationManager implements ICachedPublicClientApplicationManager {
	...
	async initialize() {
		...
		let migrateObject: Map<string, string[]> | undefined;
		try {
			migrateObject = await this._getMigrateObject();
	...
	private async _getMigrateObject(): Promise<Map<string, string[]> | undefined> {
		await this._pcasSecretStorage.migrate();
